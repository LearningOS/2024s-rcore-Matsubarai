...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        if task0.start_time == 0 {task0.start_time = get_time_ms()};
        drop(inner);
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[current].start_time == 0 {inner.tasks[current].start_time = get_time_ms()};
            drop(inner);
            // before this, we should drop local variables that must be dropped manually
            unsafe {
...
/// Increment the syscall counter of current `Running` task
pub fn inc_syscall_times(syscall_id: usize) {
    TASK_MANAGER.inc_syscall_times(syscall_id);
}

/// Get the syscall counter of current `Running` task
pub fn get_syscall_times() -> [u32; MAX_SYSCALL_NUM] {
    TASK_MANAGER.get_syscall_times()
}

/// Get the start time of current `Running` task
